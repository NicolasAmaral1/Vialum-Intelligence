    ...
    if (significantKinds.includes(entry.kind)) {
      ...
      prisma.workflowEvent.create({
        data: {
          ...
          toolName: 'name' in entry ? String((entry as Record<string, unknown>).name) : null,
          payload: entry as unknown as JsonInput,
        },
  ...
  if (isSessionAware(adapter)) {
    ...
    if (!handle) {
      handle = await (adapter as SessionAwareAdapter).startSession({
        ...
        accountId: step.accountId, adapterConfig: ((step as unknown as Record<string, unknown>).adapterConfig as Record<string, unknown>) ?? {}, cwd: '',
      });
    ...
    result = await adapter.execute({
      ...
      accountId: step.accountId, adapterConfig: ((step as unknown as Record<string, unknown>).adapterConfig as Record<string, unknown>) ?? {}, input, prompt,
